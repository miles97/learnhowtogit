...
如果是一维数组的话更加简单，直接一层就可以解决问题，加上wx:key的原因是为了增加稳定性。
...
wx:for-index给以渲染元素的idx，在筛选一些数据的时候很好用。
...
{{isActive? 'active' : '' }}，诸如此类。当然，如果硬要用,微信小程序自带了一个hover-class属性，直接编写一个class然后绑定就好，不用单独写一个:hover。
...
### 其他的开发问题主要就是关于跨页面使用的问题，基本上需要用到的数据需要wx.setstorage